...
export const AnalysisCharts: React.FC<AnalysisChartsProps> = ({ competitorData, priceTrendData, marketShareData }) => {
  const formattedCompetitorData = competitorData.map(d => ({ ...d, price: d.price / 100 }));
    ...
    <section>
      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-6">
        <ChartCard title="Competitor Price Comparison">
          <BarChart data={formattedCompetitorData} layout="vertical" margin={{ top: 5, right: 20, left: 80, bottom: 5 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
            <XAxis type="number" stroke="#9ca3af" domain={[0, 'dataMax + 5']} tickFormatter={(tick) => `$${tick}`} />
            <YAxis type="category" dataKey="name" stroke="#9ca3af" width={120} />
            <Tooltip content={<CustomTooltip />} cursor={{ fill: 'rgba(148, 163, 184, 0.1)' }}/>
            <Bar dataKey="price" name="Price" fill="#3b82f6" />
          </BarChart>
        </ChartCard>
        <ChartCard title="Price Trend Analysis">
      ...
      <ChartCard title="Market Share Analysis">
